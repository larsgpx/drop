jQuery(function($){
    ...
    var Review = {
		...
		addAnalysisTask : function ( productId, target, sub_title, time ) {
            ...
            window.DM.aliExtension
                .getPage('https://home.aliexpress.com/dropshipper/product_analysis_ajax.htm?productId='+productId)
                .then( function (value) {
    ...
    var Shipping = {
        ...
        remote_get : function (url){

            return new Promise(function(resolve, reject) {

                var country = parseURL( 'country', url ),
                    pid     = parseURL( 'productid', url );
                ...
                $.ajax({
                    ...
                    success: function ( response ) {
                        ...
                        if( response.hasOwnProperty('freight')){
                            var product_data = [];

                            $.each(response.freight, function (i, item) {

                                var args = {
                                    company   : item.companyDisplayName,
                                    country   : item.sendGoodsCountryFullName,
                                    total     : convertPrice( item.localTotalFreight ),
                                    price     : convertPrice( item.price ),
                                    s         : parseFloat( item.saveMoney ) === 0,
                                    time      : item.time,
        ...
        requestAll : function (urls) {

            var $this = this;

            Promise.all( urls.map( $this.remote_get ) ).then( function (values) {
        ...
        requestShipping : function( product_id, country ) {
            ...
            var response = [];
            var url = path + '&productid=' + product_id + '&country=' + country;
